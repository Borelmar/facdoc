...
%stitle: '%s'\n";
    ...
    printf(bl_info_pattrn, ofs, block->type, ofs, block->blocks_count, ofs, block->title);

    /* print child blocks */
    for(unsigned int i = 0; i < block->blocks_count; i++)
    {
        switch( ( (fcd_base_block*)block->blocks[i] )->type )
        {
            case(BLOCK_T_SECTION):
                print_fcd_section_block(block->blocks[i], depth+1);
                break;
            case(BLOCK_T_TEXT):
                print_fcd_text_block(block->blocks[i], depth+1);
                break;
            case(BLOCK_T_IMAGE):
                print_fcd_image_block(block->blocks[i], depth+1);
                break;
            case(BLOCK_T_CODE):
                print_fcd_code_block(block->blocks[i], depth+1);
                break;
        }
    }
    printf("%s** end bblock **\n", ofs);
    ...
    print_fcd_root_block_tree(fcd_root_block* root_block)
{
    ...
    printf("*** BEGIN ROOT ***\n");
    printf("sections count: '%d'\n", root_block->sections_count);
    for(unsigned int i = 0; i < root_block->sections_count; i++)
    {
        printf("** section '%d' **", i);
        print_fcd_section_block(root_block->sections[i], depth);
        printf("** end section '%d' **", i);
    }
    printf("*** END ROOT ***\n");
}
